    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (await userManager.IsInRoleAsync(await userManager.FindByEmailAsync(model.Email), "ADMIN"))
                    {
                        return RedirectToAction("Index", "Admin");
                    }
                    else if (await userManager.IsInRoleAsync(await userManager.FindByEmailAsync(model.Email), "USER"))
                    {
